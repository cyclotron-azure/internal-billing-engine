...
Breaker-Load -Path $StateFile -Chain 'claude-sonnet-5,claude-haiku-4-5-20251001'
...
function Invoke-LoopFillSlot {
    ...
    if ($Worktree -eq 1) {
        ...
        # second probe is a DIRECTORY test on .claude, deliberately NOT
        ...
        # .claude in these single-copy loop drivers resolves to the
        ...
        # directory probe on .claude is true on all four harnesses
        ...
        $ideDirOk = Test-Path -LiteralPath (Join-Path $wt '.claude') -PathType Container
        if (-not $backlogOk -or -not $ideDirOk) {
            [Console]::Error.WriteLine("!!! worktree preflight failed: '$wt' is missing a tracked _goals/backlog.md and/or a tracked .claude directory — a worktree checks out TRACKED files only, so this repo's loop instance must be committed (not gitignored) for LOOP_WORKTREE=1 — stopping, no fallback to the main tree")
