---
name: test-ladder
description: Staged test ladder for internal-billing-engine — run new tests first, then impacted tests, and the full suites only after all changes in the cycle are applied. Use whenever writing, running, or evaluating tests.
user-invocable: true
...
Rung 2  Impacted tests     →  existing tests that exercise the files you changed
            │ pass
            ▼
Rung 3  Full suites        →  python -m pytest -q
...
Tests live in `tests/` at the repo root, mirroring the module name. All commands run
from the repo root — no `cd` is required, and `python -m pytest` (not a bare `pytest`)
keeps the repo root on `sys.path` so `import billing` resolves without an installed
package.
...
That is the whole of rung 3 — this is a single-stack Python repo, so there is one
full-suite command and no second stack to run. **No linter and no type checker are
configured**; do not invent `ruff`, `flake8`, `black`, or `mypy` invocations at cycle
end. If one is adopted later, add it here and to the Quality Checks table in
`.claude/ORCHESTRATION.md` in the same change.
