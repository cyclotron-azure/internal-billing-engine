---
name: task-criteria
description: Criteria the evaluator applies when evaluating a completed task in internal-billing-engine. Reference skill — not invoked directly.
...
(`python -m pytest tests/test_<name>.py -v`-style rung 1–2 run) — do not trust reported output.
...
Apply the auto-fail list in `.claude/agents/evaluator.md`.
